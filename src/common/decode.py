...
def read_nnet3_model(model_path):# -> nnet3.Nnet:
...
def read_nnet3Computer_model (nnet):
    nnetComputeOptions = nnet3.NnetComputeOptions()
    nnetComputation = nnet3.NnetComputation()
    nnetComputer = nnet3.NnetComputer(nnetComputeOptions, nnetComputation, nnet, None)
    return nnetComputer

def read_trans_model(model_path): #-> hmm.TransitionModel:
...
def read_den_fst(fst_path): #-> fstext.StdVectorFst:
