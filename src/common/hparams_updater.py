...
def main():
	...
	with open(hparams_file) as infile:
			hparams_dict=json.load(infile)

	hparams_dict["n_symbols"]=259
	hparams_dict['ppg_fdir']=hparams_dict['ppg_fdir'].replace('ppg','ppg_reduce')
	
	with codecs.open(hparams_file, 'w','utf-8') as outfile:
		json.dump(hparams_dict,outfile)
