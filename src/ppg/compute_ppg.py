...
from kaldi.nnet3 import Nnet, NnetComputer, NnetIo
...
def compute_full_ppg_nnetc (nnet: Nnet, nnetC: NnetComputer, feats, ivectors) -> Matrix:

    """feats_cu = CuMatrix()
    feats_cu.copy_from_mat(feats)
    ivects_cu = CuMatrix()
    ivects_cu.copy_from_mat(ivectors)

    nnetC.accept_input("input", feats_cu)
    nnetC.accept_input("ivectors", ivects_cu)"""
    feats_io = NnetIo()
    feats_io.name = "input"
    feats_io.from_matrix("input", 0, feats)
    ivects_io = NnetIo()
    ivects_io.name = "ivectors"
    ivects_io.from_matrix("ivectors", 0, ivectors)

    nnetC.accept_inputs(nnet, [feats_io, ivects_io])
    full_ppg_cu = nnetC.get_output_destructive("output-xent")
    nnetC.run()



    full_ppg = Matrix(full_matrix_cu)
    return full_ppg

def compute_full_ppg_softmax (nnet: Nnet, feats, ivectors):

    cr = nnet3.ComputationRequest()

    inputs = []
    outputs = []

    iospec = nnet3.IoSpecification()
    inputs.append(iospec.from_interval("input", 0, 40))
    inputs.append(iospec.from_interval("ivector", 0, 100))
    outputs.append(iospec.from_interval("output-xent", 0, 3536))

    cr.inputs = inputs
    cr.outputs = outputs


    cr.index_for_output("output-xent")

    opts = nnet3.NnetSimpleComputationOptions()
    opts.acoustic_scale = 1.0
    compiler = nnet3.Compiler(cr, nnet)
#compiler = nnet3.CachingOptimizingCompiler.new_with_optimize_opts(nnet, opts.optimize_config)
    #compiler_opts = nnet3.CompilerOptions()
    #compiler_opts.output_debug_info = True
    #computation = compiler.compile(cr)
    #computation.compute_cuda_indexes()
    computation_opts = nnet3.NnetSimpleComputationOptions()
    #computer = nnet3.NnetComputer(computation_opts, computation, nnet, None)
    priors = Vector()
    nnet_computer = nnet3.DecodableNnetSimple(computation_opts, nnet, priors, feats=feats,online_ivectors=ivectors,online_ivector_period=10,compiler=compiler)
    ...
    #computer.accept_inputs(nnet, [feats_io, ivects_io])
    #computer.run()
    #full_ppg_cu = computer.get_output_destructive("output-xent")
    raw_ppgs = Matrix(nnet_computer.num_frames(), nnet_computer.output_dim())
    #full_ppg = Matrix(full_ppg_cu)
...
def compute_full_ppg_chain(nnet: Nnet, feats) -> Matrix:
    ...
    nnet_computer = nnet3.DecodableNnetSimple(opts, nnet, priors, feats=feats[0],online_ivectors=feats[1],online_ivector_period=10,compiler=compiler)
...
def compute_feat_for_nnet_internal(wav: WaveData, lda: Matrix,
    ...
    """
    options = {"is_use_energy": True, "is_downsample": True, "frame_shift": 10,
               "is_snip_edges": True, "left_context": 3, "right_context": 3}
...
class DependenciesPPG(object):
    ...
    def __init__(self,
        ...
        print(' lda {} nnet {}  reduce {}  splice {}'.format(lda_path,nnet_path,reduce_dim_path,splice_opts_path))
        ...
        if  os.path.exists(reduce_dim_path):
            self.monophone_trans = feat.read_sparse_mat(reduce_dim_path)
        else:
            print('new reduce mat')
            self.monophone_trans=SparseMatrix().from_dims(40,3536)
            self.monophone_trans.set_randn_(0.04)
        
        
        
        print(self.monophone_trans.num_rows,self.monophone_trans.num_cols)
