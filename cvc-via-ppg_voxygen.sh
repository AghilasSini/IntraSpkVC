...
test_data_set=/vrac/asini/workspace/voice_conversion/IntraSpkVC/data_fr/filelists/Voxygen_demo/test_set.txt
if [ -f $teacher_utterance_path ];then
	rm $teacher_utterance_path
fi

for fl in `cat ${test_data_set}`;do
	testFile=/vrac/asini/workspace/voice_conversion/IntraSpkVC/data_fr/ppg/`echo ${3}`/`basename ${fl%.wav}`_ppg.npy
	echo $testFile
	#echo $testFile
	if [ -f $testFile ];then
		echo $testFile
