...
if test "$#" -ne 4; then 
	echo "##########################"
	echo "Usage:"
	echo "./cvc-via-ppg_grifandlim.sh <ppg_mode_path> <sample_ppg.npy> <target_speaker> <wav_test_list> "
	exit 1
fi
...
test_data_set=${4}   #${PROJECT_ROOT_DIR}/data_fr/filelists/${3}/test_set.txt
#if [ -f $teacher_utterance_path ];then
#	rm $teacher_utterance_path
#fi

for fl in `cat ${test_data_set}`;do
	testFile=/vrac/asini/workspace/voice_conversion/IntraSpkVC/data_fr/one_hot/${3}/`basename ${fl%.wav}`_ppg.npy
	#echo $testFile
	if [ -f $testFile ];then
...
waveglow_model=/vrac/asini/workspace/voice_conversion/pretrained_models/waveglow_256channels_universal_v5.pt
